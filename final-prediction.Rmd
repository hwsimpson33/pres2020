...
states.model <- data.frame(my.names) %>% rename(state = my.names)
loop <- 1

for(df in split.data) {
  my.state <- paste(my.names[loop])
  my.model <- lm(rlead.vote ~ rlead.prev + rlead.030, data = df)
  my.state.model <- tidy(my.model)
  my.state.model <- state.model %>%
    mutate(state = my.state)
  #state.predict <- predict(my.model, df[df$year == 2020])
  states.model <- merge(states.model, state.model, by = c("state"), all = TRUE)
  loop <- loop + 1
}

states.model <- states.model %>% arrange(state, term)
```