...
```{r}
state.county.data <- merge(state.turnout, county.filter, by = c("state", "year"), all = TRUE)

# my.multi.model <- lmer(D_win_margin ~ unem.diff + (1 + turnout.pct|fips), data = state.county.data, REML = FALSE)
