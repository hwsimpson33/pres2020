...
ads.model <- ads.state.county %>% filter(year != 2016) %>% mutate(ave.030 = case_when(is.na(ave.030) ~ ave.30plus, !is.na(ave.030) ~ ave.030)) %>% 
  group_by(state_abb) %>%
  do(model = lm(D_win_margin ~ turnout.pct + year + lag.dmargin + White + age65 + ave.030 + ads.diff, .)) %>% 
  rowwise() %>% tidy(model)
...
ads.test.model <- offices.ads.model %>% select(state_abb, r.squared, adj.r.squared)
no.turnout.ads.test.model <- no.turnout.model %>% select(state_abb, r.squared, adj.r.squared) %>% rename(no.turnout.r.squared = r.squared, no.turnout.adj.r.squared = adj.r.squared)
