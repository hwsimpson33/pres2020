...
basic.model.dem <- county.filter %>% filter(D_win_margin > 0) %>% lm(D_win_margin ~ turnout.pct, .)
summary(basic.model.dem)

basic.model.rep <- county.filter %>% filter(D_win_margin < 0) %>% lm(D_win_margin ~ turnout.pct, .)
...
polls.model <- state.county.data %>%
  group_by(state_abb) %>%
  do(model = lm(D_win_margin ~ turnout.pct + year + lag.dmargin + White + age65 + ave.030, .)) %>% 
  rowwise() %>% tidy(model)
...
polls.ec.model <- state.county.data %>%
  group_by(state_abb) %>%
  do(model = lm(D_win_margin ~ turnout.pct + year + lag.dmargin + White + age65 + ave.030 + unem.diff, .)) %>% 
  rowwise() %>% tidy(model)
...
ads.model <- ads.state.county %>% filter(year != 2016) %>% mutate(ave.030 = case_when(is.na(ave.030) ~ ave.30plus, !is.na(ave.030) ~ ave.030)) %>% 
  group_by(state_abb) %>%
  do(model = lm(D_win_margin ~ turnout.pct + year + lag.dmargin + White + age65 + unem.diff + ads.diff + ave.030 , .)) %>% 
  rowwise() %>% glance(model)

r2.plot <- ads.model %>% filter(adj.r.squared != "NaN", adj.r.squared != 0) %>% ggplot(aes(x = adj.r.squared)) + 
  geom_histogram(bins = 10, fill = "#404040") + 
  theme_classic() + 
  theme(axis.line = element_line(colour = "#E1E1E1")) + labs(x = "Adjusted R Squared (each observation is one state-level regression", y = "Frequency", title = "Distribution of R Squared", subtitle = "democratic win margin = county-level turnout + lagged democratic win margin + year fixed effects + \n% White + % over 65 + year-on-year Q2 unemployment + poll average from last 30 days of race + \nstate difference in ad spending", caption = "Some states are excluded because of missing data. Includes data from 2000-2012.")
r2.plot
...
no.turnout.ads.model <- ads.state.county %>% filter(year != 2016) %>% mutate(ave.030 = case_when(is.na(ave.030) ~ ave.30plus, !is.na(ave.030) ~ ave.030)) %>% 
  group_by(state_abb) %>%
