...
ads.data <- ads.campaigns %>% 
  ...
  select(party, date, day.sum) %>% 
  distinct() %>% 
  pivot_wider(names_from = party, values_from = day.sum) %>% 
  ...
  mutate(d.ads.lead = (democrat - republican) / (democrat + republican)) %>% 
...
tv.news <- tv.news.raw %>% pivot_wider(names_from = Query, values_from = Value) %>% 
  ...
  mutate(d.news.lead = (obama - romney) / (obama + romney)) %>% 
  select(-obama, -romney)
...
time.data <- time.data %>% filter(date >= as.Date("2012-03-25"), date < as.Date("2012-11-06"))
...
#we will limit all data to after 2012-04-25, when the RNC declared Romney the presumptive nominee
```
