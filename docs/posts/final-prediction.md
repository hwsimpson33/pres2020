...
In order to validate this model, I measured the mean squared error (in-sample fit) and r<sup>2</sup> (see Table 3). Measuring out-of-sample fit is technically feasible, but the results are not very illuminating for this model because of the large number of regressions. Overall, the model does a fair job of predicting historical election results. Polls contribute most of the predictive power, with significant coefficients on previous vote share in only a few states. I chose to keep previous vote in the model because some states have few or no polls for certain years.

There are lots of ways to make this model more complicated. One could use previous county-level results and state-level polls to predict county-level vote share, and then aggregate the results to the state level. A more sophisticated approach would be to disaggregate state-level polling using county-level demographics and then use these estimates to project county-level vote share. A final way to complicate this model is to aggregate polls differently (currently, each poll in a state is weighted equally). All of these ideas require collecting data from the polls themselves, rather than pre-made datasets from a polling aggregator like [the 538 website](https://projects.fivethirtyeight.com/polls/president-general/national/), and so proved infeasible for this project. For future elections, however, I hope to explore these approaches.
