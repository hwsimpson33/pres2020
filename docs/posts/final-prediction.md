...
In order to validate this model, I measured the mean squared error (in-sample fit) and r<sup>2</sup>. Measuring out-of-sample fit is technically feasible, but the results are not very illuminating for this model because of the large number of regressions. Overall, this model does a fair job of predicting historical election results. Polls contribute most of the predictive power, with significant coefficients on previous vote share in only a few states. I chose to keep previous vote in the model because some states have few or no polls for certain years.
