...
pvstate_nona <- pvstate_df %>% 
  mutate(fake_var = 0) %>% #it won't let me run an empty regression - so I'll regress against zero
  filter(!is.na(R_pv2p)) %>% 
  select(state, year, R_pv2p, fake_var)

reg_list <- lmList(R_pv2p ~ fake_var | state, data = pvstate_nona) #generates list of means

test_data <- pvstate_nona %>% filter(state == "Alabama")

reg_test <- lm(R_pv2p ~ year, data = test_data)

dwtest(lm(R_pv2p ~ year, data = test_data), alternative = c("two.sided"))
